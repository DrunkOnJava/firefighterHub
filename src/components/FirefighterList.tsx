...
import { Users, RefreshCw, Check, X, ArrowUpDown, Trash2, UserX, Repeat, RotateCcw, Eye, Search, CheckSquare, Square, Download, FileDown, CalendarPlus, Filter } from 'lucide-react';
...
export function FirefighterList({
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (showExportMenu && !(event.target as Element).closest('[aria-label="Export roster data"]')) {
  ...
  function handleDrop(e: React.DragEvent, targetFirefighterId: string) {
    ...
    if (draggedId && draggedId !== targetFirefighterId) {
      ...
      const targetIndex = localFirefighters.findIndex(ff => ff.id === targetFirefighterId);
  ...
  function handleBulkDelete() {
    if (!confirm(`Delete ${selectedIds.size} selected firefighters?\n\nThis action cannot be undone.`)) return;
  ...
  function handleExportCSV() {
    const filename = exportRosterToCSV(firefighters, firefighters[0]?.shift || 'ALL');
    setShowExportMenu(false);
    console.log(`Exported roster to ${filename}`);
  }

  function handleExportJSON() {
    const filename = exportRosterToJSON(firefighters, firefighters[0]?.shift || 'ALL');
    setShowExportMenu(false);
    console.log(`Exported roster to ${filename}`);
  }

  return (
    <div className={`border-2 rounded-2xl shadow-2xl overflow-hidden ${isDarkMode ? 'bg-gradient-to-br from-gray-800 to-gray-850 border-gray-700' : 'bg-white border-slate-300'}`}>
      <div className={`border-b-2 p-6 ${isDarkMode ? 'bg-gradient-to-r from-gray-900 to-gray-800 border-gray-700' : 'bg-gradient-to-r from-red-50 to-amber-50 border-slate-300'}`}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className={`p-3 rounded-xl shadow-lg ${isDarkMode ? 'bg-gradient-to-br from-blue-500 to-blue-600' : 'bg-gradient-to-br from-blue-600 to-blue-700 shadow-blue-500/30'}`}>
              <Users className="text-white" size={28} />
            </div>
            <div>
              <h2 id="roster-heading" className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>Firefighter Roster</h2>
              <p className={`text-base ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>Add and organize your team rotation</p>
            </div>
          </div>
          ...
          {firefighters.length > 0 && (
            <div className="flex items-center gap-2">
              ...
              {showExportMenu && (
                <div className={`absolute right-0 top-full mt-2 w-48 rounded-lg shadow-xl border-2 overflow-hidden z-10 ${
                  isDarkMode
      ...
      <div className="p-6">
        ...
        {firefighters.length === 0 ? (
          <div className={`border-2 border-dashed rounded-xl p-12 text-center ${
            ...
            <Users className={isDarkMode ? 'text-gray-600' : 'text-slate-400'} size={64} />
            <p className={`text-lg mb-2 ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>Your roster is empty</p>
            <p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-slate-500'}`}>Add your first team member to begin scheduling holds</p>
          </div>
        ) : (
          <div className="overflow-x-auto -mx-6">
            <table className="w-full min-w-max">
              <thead>
                <tr className={`border-b-2 ${isDarkMode ? 'border-gray-700 bg-gray-900/50' : 'border-slate-300 bg-slate-50'}`}>
                  {isAdminMode && (
                  ...
                  {isAdminMode && (
                    <th className={`px-4 py-3 text-right text-xs font-bold uppercase tracking-wider ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                      Actions
                    </th>
                  )}
                </tr>
              ...
              <tbody className="divide-y divide-gray-700">
                {filteredAndAdvancedFiltered.map((firefighter, index) => {
                  const apparatusList = [
                    ...
                    { name: 'Rescue Squad', enabled: firefighter.apparatus_rescue_squad }
                  ...
                  return (
                    ...
                    >
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-center">
                          <button
                            ...
                            className={`p-1 rounded transition-colors ${
                              isDarkMode ? 'hover:bg-gray-700' : 'hover:bg-slate-200'
                            }`}
                      ...
                      <td className={`px-4 py-4 whitespace-nowrap text-sm ${isDarkMode ? 'text-gray-300' : 'text-slate-700'}`}>
                        <div className="flex items-center gap-2">
                          {isNext && (
                            <span className="px-2 py-1 bg-blue-600 text-white text-xs font-bold rounded">NEXT</span>
                          )}
                          ...
                          <div className="flex flex-wrap gap-1">
                            {apparatusList.map((apparatus) => {
                              ...
                              switch (apparatus.name) {
                                case 'Ambulance':
                                  colorClass = isDarkMode ? 'bg-red-900/80 text-white border border-red-700' : 'bg-red-600 text-white';
                                  break;
                                case 'Engine':
                                  colorClass = isDarkMode ? 'bg-orange-900/70 text-orange-300' : 'bg-orange-600 text-white';
                                  break;
                                case 'Truck':
                                  colorClass = isDarkMode ? 'bg-yellow-900/70 text-yellow-300' : 'bg-yellow-600 text-white';
                                  break;
                                case 'Tanker':
                                  colorClass = isDarkMode ? 'bg-blue-900/70 text-blue-300' : 'bg-blue-600 text-white';
                                  break;
                                case 'Brush Truck':
                                  colorClass = isDarkMode ? 'bg-green-900/70 text-green-300' : 'bg-green-600 text-white';
                                  break;
                                case 'Boat':
                                  colorClass = isDarkMode ? 'bg-cyan-900/70 text-cyan-300' : 'bg-cyan-600 text-white';
                                  break;
                                case 'UTV':
                                  colorClass = isDarkMode ? 'bg-amber-900/70 text-amber-300' : 'bg-amber-600 text-white';
                                  break;
                                case 'Rescue Squad':
                                  colorClass = isDarkMode ? 'bg-rose-900/70 text-rose-300' : 'bg-rose-600 text-white';
                                  break;
                                default:
                                  colorClass = isDarkMode ? 'bg-gray-900/70 text-gray-300' : 'bg-gray-600 text-white';
                              }
                              return (
                                <span
                                  key={apparatus.name}
        ...
        {isAdminMode && deactivatedFirefighters.length > 0 && (
          <div className="mt-6 pt-6 border-t border-gray-700">
            <h3 className={`text-sm font-bold mb-3 ${isDarkMode ? 'text-gray-400' : 'text-slate-600'}`}>
