...
describe("useFirefighters", () => {
  ...
  describe("loadFirefighters()", () => {
    it("should load active firefighters for current shift only", async () => {
      ...
      const { result } = renderHook(() =>
        useFirefighters(mockToast, "A", mockConfirmAction)
      );
      ...
      expect(result.current.firefighters.every((ff) => ff.shift === "A")).toBe(
        true
      );
      expect(
        result.current.firefighters.find((ff) => ff.id === "ff3")
      ).toBeUndefined();
    });
    ...
    it("should only load active firefighters (is_active = true)", async () => {
      ...
      expect(result.current.deactivatedFirefighters[0].name).toBe(
        "Deactivated Person"
      );
    });
  ...
  describe("completeHold()", () => {
    it("should complete a hold successfully", async () => {
      ...
      const completedFF = result.current.firefighters.find(
        (ff) => ff.id === "ff1"
      );
      ...
      const activityLog = mockDatabase.activity_log.find(
        (log) =>
          log.firefighter_id === "ff1" && log.action_type === "completed_hold"
      );
    ...
    it("should update last_hold_date when completing hold", async () => {
      ...
      const updatedFF = result.current.firefighters.find(
        (ff) => ff.id === "ff1"
      );
    ...
    it("should rollback on error", async () => {
      ...
      await waitFor(() => {
        ...
        expect(result.current.firefighters[0].order_position).toBe(
          initialState[0].order_position
        );
      });
  ...
  describe("addFirefighter()", () => {
    it("should add firefighter with correct position assignment", async () => {
      ...
      const newFF = result.current.firefighters.find(
        (ff) => ff.name === "New Person"
      );
    ...
    it("should show optimistic update immediately", async () => {
      ...
      await waitFor(
        ...
        { timeout: 100 }
      ); // Short timeout to verify it happens quickly
    });
    ...
    it("should log activity when firefighter added", async () => {
      ...
      await waitFor(() => {
        const log = mockDatabase.activity_log.find(
          (l) =>
            l.firefighter_name === "Activity Log Test" &&
            l.action_type === "added"
        );
  ...
  describe("deleteFirefighter()", () => {
    ...
    it("should log activity when firefighter deleted", async () => {
      ...
      await waitFor(() => {
        const log = mockDatabase.activity_log.find(
          (l) =>
            l.firefighter_name === "Activity Test" &&
            l.action_type === "removed"
        );
        expect(log).toBeDefined();
      });
    });
  ...
  describe("deactivateFirefighter()", () => {
    ...
    it("should log activity when firefighter deactivated", async () => {
      ...
      await waitFor(() => {
        const log = mockDatabase.activity_log.find(
          (l) =>
            l.firefighter_name === "Deactivate Test" &&
            l.action_type === "deactivated"
        );
        expect(log).toBeDefined();
      });
    });
  ...
  describe("reactivateFirefighter()", () => {
    it("should reactivate firefighter and add to position 0", async () => {
      ...
      await waitFor(() => {
        ...
        const reactivated = result.current.firefighters.find(
          (f) => f.id === "ff2"
        );
        ...
        const original = result.current.firefighters.find(
          (f) => f.id === "ff1"
        );
    ...
    it("should log activity when firefighter reactivated", async () => {
      ...
      await waitFor(() => {
        const log = mockDatabase.activity_log.find(
          (l) =>
            l.firefighter_name === "Reactivate Test" &&
            l.action_type === "reactivated"
        );
        expect(log).toBeDefined();
      });
    });
  ...
  describe("transferShift()", () => {
    it("should transfer firefighter to new shift", async () => {
      ...
      await waitFor(() => {
        ...
        const transferred = mockDatabase.firefighters.find(
          (f) => f.id === "ff1"
        );
    ...
    it("should log activity when firefighter transferred", async () => {
      ...
      await waitFor(() => {
        const log = mockDatabase.activity_log.find(
          (l) =>
            l.firefighter_name === "Transfer Log Test" &&
            l.action_type === "shift_transfer"
        );
        expect(log).toBeDefined();
