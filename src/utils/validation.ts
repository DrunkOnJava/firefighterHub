...
export function validateHoldDate(date: Date): ValidationResult {
  ...
  if (holdDate > maxFutureDate) {
    return {
      valid: false,
      error: "Cannot schedule hold more than 90 days in advance",
    };
  }
...
export function validateStatusTransition(
  ...
  if (fromStatus === "skipped") {
    ...
    return {
      valid: false,
      error: "Cannot change status of skipped hold without admin override",
    };
  }
...
export function validateFirefighterForHold(
  ...
  const firefighterHolds = existingHolds.filter(
    (h) =>
      h.firefighter_id === firefighter.id &&
      h.status !== "skipped" &&
      h.hold_date !== null
  );
  ...
  if (firefighterHolds.length >= 3) {
    ...
    if (holdsThisWeek.length >= 3) {
      return {
        valid: false,
        error: "Firefighter already has 3 holds this week",
      };
    }
  }
...
export function validateScheduleConflict(
  ...
  if (conflictingHold) {
    return {
      valid: false,
      error: `Firefighter already has a hold scheduled on ${
        newHold.hold_date.split("T")[0]
      }`,
    };
  }
...
export function validateFireStation(station: string | null): ValidationResult {
  ...
  if (isNaN(stationNum) || stationNum < 1 || stationNum > 99) {
    return {
      valid: false,
      error: "Fire station must be a number between 1 and 99",
    };
  }
...
export function validateLentToShift(
  ...
  if (firefighterShift === lentToShift) {
    return {
      valid: false,
      error: "Cannot lend firefighter to their own shift",
    };
  }
...
export function validateHoldEditable(
  ...
  if (isHoldLocked(hold) && !isAdminOverride) {
    return {
      valid: false,
      error:
        "This hold is locked (created more than 1 week ago) and cannot be edited. Contact an administrator for override.",
    };
  }
...
export function validateStartTime(
  ...
  if (!startTime) {
    return {
      valid: true,
      warning: "No start time specified, defaulting to 07:00",
    };
  }
  ...
  if (!timeRegex.test(startTime)) {
    return {
      valid: false,
      error: "Start time must be in HH:MM or HH:MM:SS format",
    };
  }
